...
class Particle(PfParticle):
    def set_likelihood(self, last_pos: np.ndarray, map: Map, strength_weight_list: np.ndarray, subject_dist_list: np.ndarray) -> None:
        ...
        if not pf_param.IS_LOST:    # don't consider match weight with old position
            if param.MATCH_WEIGHT_POLICY == 1:      # multiply maximum match weight between nearest node and nodes linked with last node
                min_cost = np.inf
                near_node_index = map.get_nearest_node(self.pos)
                for i in map.link_nodes[map.get_nearest_node(last_pos)]:
                    cost = map.get_cost(i, near_node_index)
                    if cost < min_cost:
                        min_cost = cost

                self.likelihood *= pf_util.calc_prob_weight(min_cost, 0)

            elif param.MATCH_WEIGHT_POLICY == 2:    # multiply match weight between nearest node and last node
